...
Set `cross_architecture_builds` in the environment's deployment config (or the
project's, which environments inherit and can override). It is **off by
default**: cross-builds are emulated and substantially slower, and turning them
on automatically would mean one node joining the cluster silently changed build
