...
2. **Building** — when the eligible nodes for a deployment span more than one
   architecture, the build job produces one image per architecture. The
   ...
   built for the node it lands on.
...
Without them, a cross-build fails with a message pointing at this command.
Emulated builds are substantially slower than native ones — if that matters,
restrict the environment to nodes of a single architecture with target nodes or
labels, or push a pre-built image instead.
</Note>
