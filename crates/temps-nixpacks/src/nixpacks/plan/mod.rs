...
pub struct BuildPlan {
    ...
    #[serde(rename = "staticAssets", skip_serializing_if = "Option::is_none")]
...
impl BuildPlan {
    ...
    /// Stores environment variables from project files, providers, or the build environment.
        ...
        match self.static_assets.as_mut() {
            Some(assets) => {
                for (key, value) in &static_assets {
                    assets.insert(key.clone(), value.clone());
                }
            }
            None => {
    ...
    /// Strip keys out of the Phases map before serializing with to_json.
...
mod test {
    ...
    fn test_get_phases_with_dependencies() {
        ...
        let plan = BuildPlan::new(&[setup, install, build, another], None);
