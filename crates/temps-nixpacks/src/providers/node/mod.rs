...
// Corepack < 0.34.5 hardcodes bin/pnpm.cjs and fails on pnpm 11+ (ESM entry / pnpm.mjs)
// with ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING. Pin a version that includes that fix.
const COREPACK_VERSION: &str = "0.35.0";
...
impl Provider for NodeProvider {
    ...
    fn get_build_plan(&self, app: &App, env: &Environment) -> Result<Option<BuildPlan>> {
        ...
        let mut install = Phase::install(if corepack {
            Some(format!(
                "npm install -g corepack@{COREPACK_VERSION} && corepack enable"
            ))
        } else {
                ...
                if let Some(version_str) = pkg_manager_field.strip_prefix("pnpm@") {
                    ...
                    if let Some(major_version) = version_str.split('.').next() {
                        if let Ok(major) = major_version.parse::<u32>() {
                            // nix-npm-overlay currently ships through pnpm-10_x.
                            // For pnpm 11+, Corepack installs the exact version from
                            // packageManager; the nix package is only a PATH fallback.
                            pm_pkg = match major {
                                6 => Pkg::new("pnpm-6_x"),
                                7 => Pkg::new("pnpm-7_x"),
                                8 => Pkg::new("pnpm-8_x"),
                                9 => Pkg::new("pnpm-9_x"),
                                10..=u32::MAX => Pkg::new("pnpm-10_x"),
                                _ => {
