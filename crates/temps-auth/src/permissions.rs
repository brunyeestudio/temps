...
pub enum Role {
    Admin,
    /// Platform administrator: full access to platform/user/settings administration
    /// (users, settings, system, audit, etc.) but read-only on deployable resources
    /// (projects/deployments/domains/environments). Mirrors `Role::User`, which has
    /// deploy permissions but no admin permissions.
    PlatformAdmin,
    User,
    Reader,
    ApiReader,
...
impl Role {
    ...
    pub fn permissions(&self) -> &'static [Permission] {
        match self {
            ...
            // Platform administrator: the same platform/user/settings-admin permission
            // set as `Role::Admin`, but read-only on deployable resources (Projects and
            // Deployments) -- no ability to create, modify, or delete a customer's
            // projects/deployments. See issue #25.
            Role::PlatformAdmin => &[
                Permission::AnalyticsRead,
                Permission::AnalyticsWrite,
                Permission::ApiKeysCreate,
