...
async fn node_heartbeat(
    ...
    if let Some(containers) = request.containers {
        ...
        // Run the reverse reconciliation out of band so heartbeat latency is
        // independent of the number of containers on a worker. This repairs
        // rowless containers left by old versions or a crash after Docker
        // removal began but before the owning project deletion completed.
        let orphan_state = app_state.clone();
        let orphan_node = node.clone();
        tokio::spawn(async move {
            cleanup_orphaned_node_containers(
                orphan_state.as_ref(),
                &orphan_node,
                token,
                containers,
...
async fn cleanup_orphaned_node_containers(
    app_state: &NodeAppState,
    node: &temps_entities::nodes::Model,
    token: String,
    inventory: Vec<ContainerInventoryItem>,
) {
    let deployer = match crate::cluster_ca::build_node_deployer(
        &node.address,
        token,
        node.name.clone(),
        app_state.config_service.as_ref(),
    ...
    .await
    {
        ...
        Err(error) => {
            error!(
                node_id = node.id,
                %error,
                "Failed to create node deployer for orphan reconciliation"
            );
            return;
        }
    };
    ...
    for item in inventory {
        let info = match deployer.get_container_info(&item.container_id).await {
            Ok(info) => info,
            Err(temps_deployer::DeployerError::ContainerNotFound(_)) => continue,
            Err(error) => {
                warn!(
                    node_id = node.id,
                    container_id = %item.container_id,
                    %error,
                    "Failed to inspect managed container during orphan reconciliation"
                );
                continue;
            }
        };
        ...
        if managed && info.created_at <= cutoff {
            candidates.push((info, project_id, environment_id, deployment_id));
        }
    }
    ...
    let deployment_ids: Vec<i32> = candidates
        ...
        .collect();
    let active_project_ids: std::collections::HashSet<i32> =
        match temps_entities::projects::Entity::find()
            .select_only()
            ...
            .await
        {
            ...
            Err(error) => {
                // Fail closed: an unreadable projects table is never evidence
                // that a project has been deleted.
                error!(
                    node_id = node.id,
                    %error,
                    "Failed to verify projects during orphan reconciliation"
                );
                return;
            }
        };
    let active_environment_ids: std::collections::HashSet<i32> =
        match temps_entities::environments::Entity::find()
            .select_only()
            ...
            .await
        {
            Ok(environment_ids) => environment_ids.into_iter().collect(),
            Err(error) => {
                error!(node_id = node.id, %error, "Failed to verify environments during orphan reconciliation");
                return;
            }
        };
    let existing_deployments = match temps_entities::deployments::Entity::find()
        .filter(temps_entities::deployments::Column::Id.is_in(deployment_ids))
        .all(app_state.db.as_ref())
        .await
    {
        Ok(deployments) => deployments,
        Err(error) => {
            error!(node_id = node.id, %error, "Failed to verify deployments during orphan reconciliation");
            return;
        }
    };
    ...
    for (container, project_id, environment_id, deployment_id) in candidates {
        ...
        match deployer.remove_container(&container.container_id).await {
            Ok(()) | Err(temps_deployer::DeployerError::ContainerNotFound(_)) => {
                info!(
                    ...
                    "Removed orphaned application container from worker node"
                );
            }
            Err(error) => {
                error!(
                    ...
                    container_id = %container.container_id,
                    %error,
                    "Failed to remove orphaned application container from worker node"
                );
            }
        }
    }
